...
const CARDS = [
  ...
  { img: "41d47a27-1905-4f2f-abce-1b30a1078e40", glow: false, tag: "+2.1",               tagFr: undefined,                name: "Théo · 19",  from: "5.8", to: "7.9" },
  { img: "346a19c4-5339-4af5-b66f-13a05e751c9d", glow: false, tag: "+3.0",               tagFr: undefined,                name: "Adrien · 24", from: "5.2", to: "8.2" },
...
export default function Home() {
  ...
  const getCoverStyle = useCallback(
    (index: number): React.CSSProperties => {
      ...
      const filterVal = isCenter ? "none" : abs === 1 ? "blur(3px)" : "blur(6px)";
      const visibility: "visible" | "hidden" = abs > 1 ? ("hidden" as const) : ("visible" as const);
      const pointerEvents: "auto" | "none"   = isCenter ? ("auto" as const) : ("none" as const);
  ...
  useEffect(() => {
    ...
    // Tag individual section elements
    root.querySelectorAll<HTMLElement>(
      ".section-eyebrow, .section-h, .section-lede"
    ).forEach(el => el.classList.add("reveal"));

    root.querySelectorAll<HTMLElement>(".final-cta > h2, .final-cta > p, .final-cta-buttons")
      .forEach(el => el.classList.add("reveal"));

    root.querySelectorAll<HTMLElement>(".carousel, .carousel-dots")
      .forEach(el => el.classList.add("reveal"));

    // Staggered groups
    root.querySelectorAll<HTMLElement>(".features-strip, .stats-grid, .how-grid")
      .forEach(el => el.classList.add("reveal-group"));

    // Hero children reveal immediately
    const heroGrid = root.querySelector<HTMLElement>(".hero-grid");
    if (heroGrid) {
      heroGrid.querySelectorAll<HTMLElement>(":scope > *").forEach(el =>
    ...
    // Hero is above fold — reveal immediately
  ...
  return (
    <div className="landing-pg" ref={landingRef}>
            ...
            {!authLoading && user ? (
              <button
                ...
                style={{
                  ...
                  padding: "0 14px", borderRadius: 999, border: "1px solid var(--line-2)",
        ...
        <div className="wrap hero-grid">
          <div>
            <div className="trust">
              ...
              <span>
                {fr
                  ...
                  : <>Loved by 240K+ users with <span className="star">★</span> 4.9 rating</>}
              </span>
            </div>

            <h1 className="hero-title">
              <span className="meet">{fr ? "Découvrez FaceUP" : "Meet FaceUP"}</span>
              {fr
            ...
            <p className="hero-sub">
              {fr
                ? "FaceUP est l'app IA d'analyse faciale instantanée. Téléchargez une photo, obtenez votre score, identifiez vos points faibles et révélez votre meilleure version."
                : "Meet FaceUP, the AI-powered app for instant facial analysis. Upload a photo, get your score, find your weak points and unlock your best self."}
            </p>
          ...
          <div className="phones">
              ...
              <div className="phone-screen">
                <div className="scan-screen">
                  ...
                  <div className="scan-bottom">
                    <div
                      className="scan-pill"
                      style={{ padding: "10px 12px 7px", gap: 6, width: 91, lineHeight: 1.45, letterSpacing: "0.1px", margin: -1, height: 48, fontSize: 12 }}
                    >
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                ...
                <div className="result-screen">
                  ...
                  <div className="result-card">
                    ...
                    <div className="potential-row">
                      <span>{fr ? "High Tier" : "High Tier"}</span>
                      <em>{fr ? "→ Elite" : "→ Elite"}</em>
                    </div>
            ...
            <div className="callout callout-3">
              <div className="l">{fr ? "Inclinaison" : "Canthal Tilt"}</div>
      ...
      <section className="section" id="transformations">
        <div className="wrap">
          ...
          <p className="section-lede">
            {fr
              ? "De vrais utilisateurs, de vraies évolutions. Chaque transformation suit le plan d'action généré par FaceUP après votre scan. Re-scannez chaque jour et regardez votre score grimper."
              : "Real users, real glow-ups. Each transformation comes from following the action plan FaceUP generates after your scan. Re-scan daily. Watch your score climb."}
          </p>
          ...
          >
            <div className="carousel-track">
              {CARDS.map((card, i) => (
                <div key={i} className="ba-card" style={getCoverStyle(i)}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={card.img} alt="Before and after" />
                  <div className={`ba-tag${card.glow ? " glow" : ""}`}>
                    <span className="dot" />
                    <span>{fr && card.tagFr ? card.tagFr : card.tag}</span>
                  </div>
                  <div className="ba-meta">
                    <div className="name">{card.name}</div>
                    <div className="score">
                      <span className="v">
                        {card.from} <span className="arrow">→</span> <b>{card.to}</b>
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="features-strip">
            <div className="feat-strip">
              ...
              <p>{fr
                ? "Prenez une photo. Plus de 40 points, ratios et vecteurs de symétrie traités en moins de 8 secondes."
                : "Snap a photo. 40+ landmarks, ratios and symmetry vectors processed in under 8 seconds."}</p>
            </div>
            <div className="feat-strip">
              <div className="feat-strip-ico">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              ...
              <p>{fr
                ? "12 catégories notées plus les centiles. Voyez exactement où vous en êtes et ce qui fait grimper votre score."
                : "12 categories scored, plus percentile rankings. See exactly where you stand and what raises your score."}</p>
            </div>
            <div className="feat-strip">
              <div className="feat-strip-ico">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              ...
              <p>{fr
                ? "Un plan d'action concret adapté à vos traits. Suivez votre progression chaque jour."
                : "A concrete action plan tailored to your features. Track progress as you re-scan each day."}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="stats-section" id="research">
        <div className="wrap">
          ...
          <p className="section-lede">
            {fr
              ? "Des décennies de recherche montrent que l'apparence du visage façonne la vie quotidienne, de façon mesurable. FaceUP transforme ces signaux en actions concrètes."
              : "Decades of research show that facial appearance shapes daily life in measurable ways. FaceUP turns those signals into something you can actually act on."}
          </p>
          <div className="stats-grid">
            {(
              [
                {
                  num: "100", unit: "ms",
                  textEn: "Time strangers need to form a stable first impression of trust and competence from a face alone.",
                  textFr: "Le temps qu'il faut à un inconnu pour se forger une impression stable de confiance et de compétence à partir du visage seul.",
                  src: "Willis & Todorov, 2006",
                },
                {
                  num: "+14", unit: "%",
                  textEn: "Average salary premium associated with above-average appearance, across men and women.",
                  textFr: "Prime salariale moyenne associée à une apparence au-dessus de la moyenne, chez les hommes comme chez les femmes.",
                  src: "Hamermesh, 2011",
                },
                {
                  num: "2.3", unit: "x",
                  textEn: "More callback interviews for identical resumes paired with attractive photos.",
                  textFr: "Plus de rappels pour des entretiens, à CV identique, avec une photo plus attractive.",
                  src: "Ruffle & Shtudiner, 2015",
                },
                {
                  num: "68", unit: "%",
                  textEn: "Of observers automatically rate attractive faces as more intelligent and sociable.",
                  textFr: "Des observateurs jugent automatiquement les visages attractifs plus intelligents et sociables.",
                  src: "Dion et al., 1972",
                },
              ] as { num: string; unit: string; textEn: string; textFr: string; src: string }[]
            ).map((s, i) => (
        ...
        <div className="wrap">
          ...
          <div className="how-grid">
            <div className="how-card">
              ...
              <p>{fr
                ? "Une photo de face nette suffit. Les images sont chiffrées et supprimées après 24 h."
                : "A clean front-facing shot is all we need. Photos are encrypted and auto-deleted after 24h."}</p>
            ...
            <div className="how-card">
              ...
              <p>{fr
                ? "Recevez un Score Face global et 12 sous-scores. Chaque chiffre est ancré sur un centile."
                : "Receive an overall Face Score plus 12 sub-scores. Every number is anchored to a percentile."}</p>
              <div className="how-illu">
                <svg width="100%" height="60" viewBox="0 0 200 60" fill="none">
            ...
            <div className="how-card">
              ...
              <p>{fr
                ? "Suivez votre feuille de route personnalisée. Re-scannez chaque jour. La plupart gagnent +6 points en 90 jours."
                : "Follow your personalized roadmap. Re-scan daily. Most users gain +6 points within 90 days."}</p>
              <div className="how-illu">
                <svg width="100%" height="60" viewBox="0 0 200 60" fill="none">
      ...
      <footer>
        <div className="wrap">
          ...
          <div className="foot-bot">
            <div>
              {fr
                ? "© 2026 FaceUP Labs. Photos traitées localement quand c'est possible."
                : "© 2026 FaceUP Labs. Photos processed locally when possible."}
            </div>
